...
# Aqui, estamos configurando para 60 segundos
# (isso significa que uma nova mensagem sera enviada a cada 60 segundos).
last_message = 0
message_interval = 60
...
def connect_mqtt():
    ...
    client = MQTTClient(client_id, mqtt_server, 1883,
                        user="USUARIO-DO-BROKER", password="SENHA-DO-BROKER")
    client.connect()
...
def read_sensor():
    try:
        ...
        for rom in roms:
            ...
            temp = ds_sensor.read_temp(rom)
            # mensagem = json.dumps({"id": "MGS-temp-quarto", "data": temp})
            mensagem = json.dumps(
                {"id": "MGS-temp-quarto", "data": '{0:.1f}'.format(temp)})
...
def read_api():
    try:
        ...
        umidade = parsed["humidity"]
        vento = parsed["wind_speedy"]
        velocidadeVento = ""
        ...
        mensagem2 = json.dumps(
            {"id": "MGS-temp-SLS", "data": '{0:.1f}'.format(temperatura)})
        mensagem3 = json.dumps(
            {"id": "MGS-umidade-SLS", "data": '{0:.1f}'.format(umidade)})
        mensagem4 = json.dumps(
            {"id": "MGS-vento-SLS", "data": '' + str(velocidadeVento)})
        if(isinstance((temperatura and umidade), (int or float)) and isinstance(velocidadeVento, str)):
            dados = [mensagem2, mensagem3, mensagem4]
            return dados
        else:
            return("Leituras de dados invalidas.")
    except OSError as e:
        return("Falha ao ler a API.")
    ...
    try:
        ...
        if (time.time() - last_message) > message_interval:
            ...
            for mensagem in dados:
                client.publish(topic_pub_temp, mensagem)
